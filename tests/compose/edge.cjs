'use strict';
// Only used by the isolated Compose CI overlay. Excluded from production Docker build contexts.
...
const issuer = 'https://identity.example.test:8443/realms/staff';
const callback = 'https://console.example.test:8443/auth/callback';
const codes = new Map();
...
async function form(req) {
  let body = '';
  for await (const chunk of req) { body += chunk.toString(); if (body.length > 65536) throw new Error('Oversized fixture request'); }
...
https.createServer({ key: fs.readFileSync('/fixtures/tls.key'), cert: fs.readFileSync('/fixtures/tls.crt') }, async (req, res) => {
  try {
    if (req.headers.host === 'console.example.test:8443') {
      const upstream = http.request({ hostname: 'web', port: 3006, path: req.url, method: req.method,
        headers: { ...req.headers, 'x-forwarded-proto': 'https', 'x-forwarded-host': req.headers.host } }, (response) => {
        res.writeHead(response.statusCode, response.headers); response.pipe(res);
      });
      upstream.on('error', () => { if (!res.headersSent) json(res, 502, { status: 'unavailable' }); else res.destroy(); });
      req.pipe(upstream);
      return;
    }
    ...
    if (req.method === 'GET' && url.pathname === '/realms/staff/protocol/openid-connect/auth') {
      ...
      res.writeHead(302, { location: `${callback}?${new URLSearchParams({ code, state: q.get('state') })}`, 'cache-control': 'no-store' });
      return res.end();
    }
    if (req.method === 'POST' && url.pathname === '/realms/staff/protocol/openid-connect/token') {
      const body = await form(req);
      const entry = codes.get(body.get('code')); codes.delete(body.get('code'));
      ...
      const token = random();
      tokens.set(token, { active: true, iss: issuer, aud: 'mobicred-console', sub: 'compose-test-staff', exp: Math.floor(Date.now() / 1000) + 900, tenant_ids: ['tenant-a'], realm_access: { roles: ['OPS'] } });
    ...
    if (req.method === 'POST' && url.pathname === '/realms/staff/protocol/openid-connect/token/introspect') {
      const body = await form(req);
      if (body.get('client_id') !== 'mobicred-console' || body.get('client_secret') !== process.env.TEST_CLIENT_SECRET) return json(res, 401, {});
      const claims = tokens.get(body.get('token'));
      return json(res, 200, claims && claims.exp > Date.now() / 1000 ? claims : { active: false });
    }
    return json(res, 404, {});
  } catch { return json(res, 400, {}); }
}).listen(8443, '0.0.0.0', () => console.log('Isolated TLS edge and identity fixture ready'));
