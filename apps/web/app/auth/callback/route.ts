...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    // Identity comes from authoritative introspection, never an unverified decoded token.
    ...
    // Log only our fixed stage name and numeric HTTP status. Never log codes,
    // tokens, cookies, credentials, tenant identifiers or upstream response bodies.
    console.warn('Console sign-in failed', { stage, status: error instanceof ApiFailure ? error.status : undefined });
    return finish('/auth/login?error=signin');
  }
}
