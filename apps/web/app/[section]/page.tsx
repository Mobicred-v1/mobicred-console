...
export default async function SectionPage({ params, searchParams }: { params: Promise<{ section: string }>; searchParams: Promise<CasePageQuery> }) {
  ...
  if (section === 'partners') { const { data, session } = await loadPartners(undefined, query); return <PartnerWorkspace key={JSON.stringify(query) + session?.contextVersion} data={data} session={session} />; }
