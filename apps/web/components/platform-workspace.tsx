...
export function PlatformWorkspace({ section, data, initialRecord }: { section: PlatformSection; data: ConsoleData; session?: ConsoleSession; initialRecord?: ConsoleRecord }) {
  ...
  return <>
    ...
    {data.state === 'live' ? <><div className="cards-grid">{data.items.map((item) => <article className="panel service-card" key={item.id}>
      ...
      <p>{section === 'operations' ? `Console policy: ${item.fields['Effective policy']}. No reachability probe has run.` : section === 'configuration' ? `Enabled: ${item.fields.Enabled} · Configured: ${item.fields.Configured} · Permitted: ${item.fields['Permitted for your session']}` : `Workspace: ${item.fields['Working context'] ?? 'Mobicred staff'}`}</p>
