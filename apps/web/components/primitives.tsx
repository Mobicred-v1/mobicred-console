...
export function Modal({ title, children, close, drawer = false }: { title: string; children: ReactNode; close: () => void; drawer?: boolean }) {
  ...
  return <dialog ref={ref} className={drawer ? 'drawer-dialog' : 'modal'} aria-label={title} onCancel={(e) => { e.preventDefault(); close(); }} onClick={(e) => { if (e.target !== ref.current) return; const r = ref.current.getBoundingClientRect(); if (e.clientX < r.left || e.clientX > r.right || e.clientY < r.top || e.clientY > r.bottom) close(); }}><header className={drawer ? 'drawer-header' : ''}><div><div className="eyebrow">Mobicred Console</div><h2>{title}</h2></div><button className="icon-button" aria-label="Close dialog" onClick={close}><Icon name="close" /></button></header>{children}</dialog>;
}
export function EmptyState({ state, detail, retry }: { state: SourceState; detail?: string; retry?: () => void }) {
  return <div className="empty-state"><div className="empty-icon"><Icon name={state === 'unauthorized' ? 'lock' : 'database'} size={25} /></div><h2>{state === 'unauthorized' ? 'This view is not accessible' : state === 'unavailable' ? 'Records are currently unavailable' : 'No records in this workspace'}</h2><p>{detail ?? 'Please retry or contact your administrator.'}</p>{state === 'unauthorized' ? <Link className="button primary" href="/auth/login"><Icon name="lock" size={14} />Sign in</Link> : retry ? <button className="button" onClick={retry}><Icon name="refresh" size={14} />Check again</button> : null}</div>;
}
